...
const TicketTable: React.FC = () => {
    ...
    const userMap = new Map(users.map((user) => [user.id, user.username])); // Map user.id to username
    ...
    const fetchUsers = async () => {
        try {
            const response = await fetch("http://122.169.108.252:8000/users"); // Replace with your API endpoint
            if (!response.ok) {
    ...
    useEffect(() => {
        if (currentUserType && userId) {
            fetchTickets();
            fetchUsers(); // Fetch user data alongside tickets
        }
                            ...
                            {currentItems.map((ticket) => (
                                ...
                                <>
                                    <td className="border p-3 text-center">
                                        {userMap.get(ticket.userId) || "No username found"}
                                    </td>   </>)}
